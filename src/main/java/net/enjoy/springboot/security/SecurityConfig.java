  ...
  protected void configure(AuthenticationManagerBuilder auth) throws Exception {
    auth.jdbcAuthentication()
        ...
        .usersByUsernameQuery(
            "SELECT email, password, 1 as enabled FROM users WHERE email=?") // Query to fetch user
        // details by email
        .authoritiesByUsernameQuery(
            "SELECT u.email, r.name FROM users u JOIN users_roles ur ON u.id = ur.user_id JOIN roles r ON ur.role_id = r.id WHERE u.email=?") // Query to fetch user authorities by email
